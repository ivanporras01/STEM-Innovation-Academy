...
import { getShopProductEn } from "@/data/nova-shop/catalog-en";
import { NOVA_SHOP_CATEGORY_LABELS } from "@/data/nova-shop";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const localized = getShopProductEn(slug);
  return buildPageMetadata({
    title: `${localized?.name ?? product.name} | ${NOVA_SHOP.name}`,
    description: localized?.description ?? product.description,
    path: `/shop/${slug}`,
    keywords: ["NOVA Shop", localized?.name ?? product.name, product.category],
    ogImagePath: product.imagePath,
  });
}
      ...
      <section className="nova-section-cosmic relative overflow-hidden border-b border-white/10 py-14 text-white">
        <div className="nova-container relative">
          ...
          <p className="mt-3 text-sm text-nova-cyan-light/70">
            {NOVA_SHOP_CATEGORY_LABELS[product.category]} · ${product.priceUsd} USD
          </p>
        </div>
      </section>
          ...
          <div className="grid gap-10 lg:grid-cols-2">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link href={NOVA_SHOP.path} className="nova-btn-secondary border-white/20 text-white">
                  All products
                </Link>
                <Link href={NOVA_SHOP.cartPath} className="nova-btn-secondary border-white/20 text-white/70">
                  View cart →
                </Link>
              </div>
            </div>
