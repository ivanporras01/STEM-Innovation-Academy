  ...
  return (
    <DashboardShell user={session.user}>
      ...
      <section className="mb-10">
        <h2 className="mb-4 text-lg font-semibold text-nova-deep-blue">What&apos;s Next</h2>
          ...
          {nextActions.slice(0, 3).map((action) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="font-semibold text-nova-deep-blue group-hover:text-nova-cyan">
                  {action.title}
                </h3>
                <p className="mt-1 flex-1 text-sm text-nova-gray">{action.description}</p>
      ...
      {enrollments.length > 0 && (
        <section className="mb-10">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-nova-deep-blue">Mission Path Progress</h2>
                  ...
                  <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <Link
                        ...
                        className="font-semibold text-nova-deep-blue hover:text-nova-cyan"
                      >
                        {enrollment.course.title}
                      </Link>
                      <p className="mt-0.5 text-sm text-nova-gray">
      ...
      <section className="mb-10">
        <h2 className="mb-4 text-lg font-semibold text-nova-deep-blue">Explore Now Status</h2>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Object.values(PATHWAY_META).map((meta) => {
            const prog = exploreProgress(meta.experienceSlug);
            let status = "Not started";
            let statusClass = "text-nova-gray bg-nova-off-white";
            if (prog?.completedAt) {
              status = "Complete";
            ...
            return (
              <div key={meta.experienceSlug} className="nova-card flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate font-medium text-nova-deep-blue">{meta.experienceTitle}</p>
                  <p className="text-xs text-nova-gray">{meta.badge}</p>
                </div>
                <span
      ...
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-nova-deep-blue">All Mission Paths</h2>
        <Link href="/courses" className="text-sm font-medium text-nova-cyan hover:underline">
          Browse all paths →
        </Link>
      </div>
