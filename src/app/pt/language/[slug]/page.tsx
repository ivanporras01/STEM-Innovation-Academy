...
import { NOVA_LANGUAGE } from "@/lib/novahub-brand";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const localized = getLanguageCoursePt(slug);
  return {
...
export default async function PortugueseLanguageCoursePage({ params }: Props) {
  const { slug } = await params;
  const course = getNovaLanguageCourseBySlug(slug);
  if (!course) notFound();
