...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!course) return { title: "Track no encontrado" };
  return {
...
export default async function SpanishCollegeTrackPage({ params }: Props) {
  const { slug } = await params;
  const course = getNovaCollegeCourseBySlug(slug);
  if (!course) notFound();
        ...
        <div className="nova-container space-y-12">
          <div className="grid gap-6 lg:grid-cols-3">
            <section className="nova-glass-island lg:col-span-2">
              <h2 className="text-lg font-bold text-white">Resultados de aprendizaje</h2>
              ...
              {course.modules.map((mod) => (
                <article key={mod.order} className="nova-glass-card p-6">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <span className="text-xs text-nova-cyan-light/60">{mod.contactHours}h contacto</span>
                  </div>
                  <ul className="mt-4 space-y-2">
                    {mod.lessons.map((lesson) => (
                      <li key={lesson.slug}>
                        {lesson.content ? (
                          <Link
                            href={`/es/college/${slug}/lessons/${lesson.slug}`}
          ...
          {course.capstone && (
            <section className="nova-glass-island">
              <h2 className="text-lg font-bold text-white">Proyecto capstone</h2>
          ...
          <section className="nova-glass-island flex flex-col gap-4 border-nova-orange/20 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-white">
                Solicita una beca {NOVAHUB.name} para este track — revisión humana, sin garantía
                automática.
              </p>
            </div>
            <Link
              href={`/es/scholarships/apply?program=${isAdvanced ? "beca-quantum" : "techhub-beca"}`}
          ...
          {qwaDelivery && (
            <section className="nova-glass-island text-center">
              ...
              <p className="mx-auto mt-2 max-w-xl text-sm text-nova-cyan-light/80">
                Este track es parte de {NOVA_COLLEGE.name}. Simuladores, esfera de Bloch y labs
                Qiskit se ejecutan en la app de delivery {QUANTUM_WORKFORCE.shortName}.
              </p>
              <a
          ...
          <section className="mb-12">
            <CertificatePreviewPromo programTitle={course.title} locale="es" />
          </section>

          <section className="text-center">
            <p className="text-sm text-nova-cyan-light/60">
              Certificado verificable · {course.verifyCertificatePrefix} ·{" "}
              <Link href="/es/verify" className="text-nova-cyan hover:underline">
